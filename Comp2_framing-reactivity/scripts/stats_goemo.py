#!/usr/bin/env python3
"""
Statistical analysis — Tier 2 GoEmotions metrics.

Reads scored_goemo.csv and produces descriptive statistics,
per-model mixed-model ANOVAs (framing × scenario), permutation
sensitivity checks, and within-framing model comparisons.

Input:  output/score_goemo/scored_goemo.csv
Output: output/stats_goemo/
            goemo_stats_results.json       — full statistical output
            goemo_descriptive_results.json — aggregated means, SDs, deltas
            goemo_analysis_summary.txt     — human-readable log

Usage:
    python scripts/stats_goemo.py
    python scripts/stats_goemo.py --input path/to/scored_goemo.csv
    python scripts/stats_goemo.py --skip-permutation   # faster
    python scripts/stats_goemo.py --force               # overwrite
"""
...
METRIC_KEYS = ["pos_emotion", "neg_emotion"]
...
def load_scored(csv_path: str) -> list[dict]:
    """Load scored_goemo.csv, convert metric columns to float."""
    records = []
...
def main():
    parser = argparse.ArgumentParser(
        description="Statistical analysis of Tier 2 GoEmotions metrics"
    )
    parser.add_argument(
        "--input",
        type=str,
        default=None,
        help="Path to scored_goemo.csv",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("output/stats_goemo"),
        help="Output directory (default: output/stats_goemo)",
    )
    parser.add_argument(
        "--skip-permutation",
        action="store_true",
    ...
    input_path = args.input or "output/score_goemo/scored_goemo.csv"
    out_dir = args.output_dir

    # Pre-check outputs
    output_files = [
        out_dir / "goemo_stats_results.json",
        out_dir / "goemo_descriptive_results.json",
        out_dir / "goemo_analysis_summary.txt",
    ]
    if not args.force:
    ...
    for (model, framing), metrics in sorted(agg.items()):
        ...
        for m, vals in metrics.items():
            ...
            entry[m + "_mean"] = round(mean, 4)
            entry[m + "_sd"] = round(var ** 0.5, 4)
    ...
    for model, metrics in baselines_raw.items():
        ...
        baselines_json[model] = {
            k.replace("_mean_raw", "_mean"): round(sum(v) / len(v), 4)
    ...
    for e in aggregated:
        ...
        for k in list(e.keys()):
            if k.endswith("_mean_raw"):
                ...
                e[base + "_delta"] = round(e[k] - bl[k], 4)
    ...
    for model in MODELS:
        ...
        for e in sorted(model_entries, key=lambda x: x["framing"]):
            ...
            for m in METRIC_KEYS:
                row += f"  {e[m + '_mean']:10.4f}±{e[m + '_sd']:<6.4f}"
            log(row)
        log("")
    ...
    desc_path = out_dir / "goemo_descriptive_results.json"
    ...
    stats_path = out_dir / "goemo_stats_results.json"
    with open(stats_path, "w") as f:
        json.dump(stats_output, f, indent=2)

    txt_path = out_dir / "goemo_analysis_summary.txt"
